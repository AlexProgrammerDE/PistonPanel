    ...
    let _ = TrayIconBuilder::with_id("tray")
        .icon(
            app.default_window_icon()
                .ok_or(SFError::NoDefaultWindowIcon)?
                .clone(),
        )
        .menu(&menu)
        .show_menu_on_left_click(false)
        .on_menu_event(
            move |app, event| match handle_menu_event(app.clone(), event) {
                Ok(_) => {}
                Err(e) => error!("Error during menu event: {e}"),
            },
        )
        .on_tray_icon_event(|tray, event| match handle_tray_event(tray.clone(), event) {
            Ok(_) => {}
