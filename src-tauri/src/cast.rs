...
impl CastRunningState {
    ...
    pub async fn remove_discovered_device(&self, full_name: String) {
        self.announced_devices
            .lock()
            .await
            .retain(|x| x.full_name != full_name);
    }
}
        ...
        match event {
            ServiceEvent::ServiceResolved(info) => {
                if let Some(model_name) =
                    info.get_properties().get_property_val_str(CAST_MODEL_NAME)
                ...
                if cast_running_state
                    .contains_discovered_device(full_name.to_string())
                    .await
                {
                    continue;
                }
                ...
                cast_running_state
                    .add_discovered_device(DiscoveredCast {
                        id: id.to_string(),
            ...
            ServiceEvent::ServiceRemoved(_, full_name) => {
                ...
                {
                    cast_running_state
                        .remove_discovered_device(full_name.to_string())
                        .await;
...
fn create_cast_connection(
    ...
    cast_device.connection.disconnect(DEFAULT_DESTINATION_ID)?;
    cast_device.connection.connect(&application.transport_id)?;
    ...
    loop {
        ...
        match cast_device.receive() {
            Ok(ChannelMessage::Heartbeat(response)) => match response {
            ...
            Ok(ChannelMessage::Raw(response)) => {
                if response.namespace == CAST_APP_NAMESPACE {
                    ...
                    let message_type = json_message
                        .get("type")
                        .ok_or(SFError::JsonFieldInvalid("type".to_string()))?
                        .as_str()
                        .ok_or(SFError::JsonFieldInvalid("type".to_string()))?;
                    if message_type == "CHALLENGE_REQUEST" {
                        let challenge = json_message
                            .get("challenge")
                            .ok_or(SFError::JsonFieldInvalid("challenge".to_string()))?
                            .as_str()
                            .ok_or(SFError::JsonFieldInvalid("challenge".to_string()))?;
                        let response = json!({
            ...
            Ok(Connection(connection)) => match connection {
